...
export default function CommentSection({ postId }) {
  ...
  const handleLike = async (commentId) => {
    try {
      ...
      if (res.ok) {
        const responseJSON = await res.json()
        console.log(responseJSON.data, "---response json");
        ...
        setComments(
          comments.map((comment) =>
            comment._id === commentId
              ? {
                  ...comment,
                  likes: responseJSON.data.likes,
                  numberOfLikes: responseJSON.data.likes.length,
                }
              : comment
          )
        )
  ...
  const handleDelete = async (commentId) => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/comment/${commentId}`, {
        method: 'DELETE',
      });
      if (res.ok) {
        const responseJSON = await res.json();
